...
export const uploadsBase = API_URL + '/uploads';

export const getProofUrl = (proofPath) => {
  if (!proofPath) return null;
  if (proofPath.startsWith('http://') || proofPath.startsWith('https://')) return proofPath;
  return `${uploadsBase}/${proofPath}`;
};
...
api.interceptors.response.use(
  ...
  (err) => {
    if (err.response?.status === 401 && typeof window !== 'undefined' && !isRedirecting) {
      ...
      if (!window.location.pathname.startsWith('/auth') && !window.location.pathname.startsWith('/api')) {
        isRedirecting = true;
...
export const adminAPI = {
  ...
  manualCredit: (data) => api.post('/admin/deposits/manual-credit', data),
  ...
  revertWithdrawal: (id) => api.put(`/admin/withdrawals/${id}/revert`),
  revertAllWithdrawals: (status) => api.post('/admin/withdrawals/revert-all', { status }),
  deleteAllWithdrawals: (status) => api.post('/admin/withdrawals/delete-all', { status }),
  ...
  resetProducts: () => api.post('/admin/products/reset'),
  ...
  deletePaymentAccount: (id) => api.delete(`/admin/payment-accounts/${id}`),
  getSwitchHistory: () => api.get('/admin/payment-accounts/switch-history'),
  setDefaultAccount: (id) => api.put(`/admin/payment-accounts/${id}/default`),
  ...
  reseed: (securityKey) => api.post('/admin/reseed', { securityKey }),
  resetFinancials: () => api.post('/admin/reset-financials'),
  ...
  generateTasks: () => api.post('/admin/tasks/generate'),
  resetTasks: () => api.post('/admin/tasks/reset'),
};
...
export const settingsAPI = {
  ...
  getWithdrawal: () => api.get('/settings/withdrawal'),
};
...
export const leaderboardAPI = {
  getTopInvestors: () => api.get('/leaderboard/top-investors'),
  getWeeklyGrowing: () => api.get('/leaderboard/weekly-growing'),
};

export const taskAPI = {
  getToday: () => api.get('/tasks/today'),
  start: (taskId) => api.post('/tasks/start', { taskId }),
  receiveReward: (taskId) => api.post('/tasks/receive-reward', { taskId }),
